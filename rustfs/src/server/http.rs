...
/// Map HTTP method to a static string to avoid heap allocation on every request.
#[inline]
...
/// Classify an API request path into a bounded set of categories for metrics.
/// This prevents metrics cardinality explosion from unique object paths.
///
/// Categories map to the server's route prefixes defined in `prefix.rs`:
/// - `health`  — /health, /health/ready
/// - `admin`   — /rustfs/admin/...
/// - `console` — /rustfs/console/...
/// - `rpc`     — /rustfs/rpc/...
/// - `grpc`    — /node_service.NodeService/...
/// - `profile` — /profile/cpu, /profile/memory
/// - `s3_api`  — all S3-compatible object storage operations
...
mod classify_tests {
    ...
    #[test]
    fn test_classify_health_paths() {
        assert_eq!(classify_api_path("/health"), "health");
        assert_eq!(classify_api_path("/health/ready"), "health");
    }

    #[test]
    fn test_classify_admin_paths() {
        assert_eq!(classify_api_path("/rustfs/admin"), "admin");
        assert_eq!(classify_api_path("/rustfs/admin/v3"), "admin");
        assert_eq!(classify_api_path("/rustfs/admin/v3/info"), "admin");
        assert_eq!(classify_api_path("/minio/health/cluster"), "admin");
    }

    #[test]
    fn test_classify_console_paths() {
        assert_eq!(classify_api_path("/rustfs/console"), "console");
        assert_eq!(classify_api_path("/rustfs/console/login"), "console");
        assert_eq!(classify_api_path("/favicon.ico"), "console");
    }

    #[test]
    fn test_classify_rpc_paths() {
        assert_eq!(classify_api_path("/rustfs/rpc/read_file_stream"), "rpc");
    }

    #[test]
    fn test_classify_grpc_paths() {
        assert_eq!(classify_api_path("/node_service.NodeService/SomeMethod"), "grpc");
    }

    #[test]
    fn test_classify_profile_paths() {
        assert_eq!(classify_api_path("/profile/cpu"), "profile");
        assert_eq!(classify_api_path("/profile/memory"), "profile");
    }

    #[test]
    fn test_classify_s3_api_paths() {
        assert_eq!(classify_api_path("/"), "s3_api");
        assert_eq!(classify_api_path(""), "s3_api");
        assert_eq!(classify_api_path("/my-bucket"), "s3_api");
        assert_eq!(classify_api_path("/my-bucket/my-object"), "s3_api");
        assert_eq!(classify_api_path("/bucket/prefix/deep/object.txt"), "s3_api");
    }
}
            ...
            .layer(
                TraceLayer::new_for_http()
                    ...
                    .on_request(|request: &HttpRequest<_>, span: &Span| {
                        ...
                        let api_type = classify_api_path(request.uri().path());
                        let method_str = method_to_static_str(request.method());
                        let labels = [
                            ("key_request_method", method_str.to_string()),
                            ("key_api_type", api_type.to_string()),
                        ];
                    ...
                    .on_response(|response: &Response<_>, latency: Duration, span: &Span| {
                        ...
                        counter!("rustfs.api.responses.total", "key_status_class" => status_class.to_string()).increment(1);
