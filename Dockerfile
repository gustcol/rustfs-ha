...
# Kubernetes uses its own liveness/readiness probes configured in the Helm chart.
HEALTHCHECK --interval=30s --timeout=5s --start-period=30s --retries=3 \
    CMD curl -f http://localhost:9000/health || exit 1

# Properly handle SIGTERM for graceful shutdown in container orchestrators.
# The rustfs binary already handles SIGTERM via tokio signal handlers.
STOPSIGNAL SIGTERM
